...
//Loc1 = 0, Loc2 = 1
const char* forecast_1 = "";
const char* current_1 = "";
const char* alerts_1_Z = "";
const char* alerts_1_C = "";

const char* forecast_2 = "";
const char* current_2 = "";
const char* alerts_2_Z = "";
const char* alerts_2_C = "";
...
void printLocalTime() {
  ...
  Serial.print("1 " + String(hourt) + " " + String(min) + " " + String(sec) + " C[" + String(curTempF_1) + "] F[" + String(preTempF_1) + "] D[" + String(dif_1) + "] P[" + String(twoPOP_1) + "] WC[" + String(curChillF_1) + "] HI[" + String(curHeatF_1) + "] BA[" + String(curBarohPa_1) + "]   ");
  Serial.println("2 " + String(hourt2) + " " + String(min) + " " + String(sec) + " C[" + String(curTempF_2) + "] F[" + String(preTempF_2) + "] D[" + String(dif_2) + "] P[" + String(twoPOP_2) + "] WC[" + String(curChillF_2) + "] HI[" + String(curHeatF_2) + "] BA[" + String(curBarohPa_2) + "]   ");
}
...
void alerts(String link1, String link2, int loc) {
  allevent = "";

  for (int i = 0; i < 2; i++) {
    String linkUsed = "";
    if (i == 0) linkUsed = link1;
    if (i == 1) linkUsed = link2;
    if (linkUsed.length() == 0) return;

    //Serial.println(linkUsed);

    HTTPClient http3;
    http3.begin(linkUsed);
    ...
    while (htmlSourceCode3.indexOf("properties") > 0) {
      ...
      toAdd = headline + " -> " + location + "\\n";
      if (allevent.indexOf(toAdd) < 0 && toAdd.indexOf("Beach Hazards Statement") == -1) {
        allevent = allevent + toAdd;
      }
